...
@app.command(name="purge-cache", help="Purge cache for a specific zone.")
def purge_cache(
    ctx: typer.Context,
    zone_id: str = typer.Argument(..., help="The ID of the zone."),
    all_cache: bool = typer.Option(False, "--all", help="Purge all cache."),
    files: Optional[str] = typer.Option(
        None, "--files", help="Comma-separated URLs of files to purge."
    )
):
    client = ctx.obj["client"]
    zone_service = ZoneService(client)
    
    if not all_cache and not files:
        typer.secho("Error: You must specify either --all or --files <urls>", fg=typer.colors.RED, err=True)
        raise typer.Exit(code=1)
        
    purge_files = None
    if files:
        purge_files = [f.strip() for f in files.split(",")]
        
    try:
        zone_service.purge_cache(zone_id, files=purge_files)
